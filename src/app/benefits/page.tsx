...
async function getBenefits() {
  ...
  const { data } = await supabase.from('benefits').select('*').order('category')
  return data ?? []
}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <Link
                ...
                className="group bg-white/[0.03] hover:bg-white/[0.05] border border-white/[0.07] hover:border-gold-500/30 rounded-2xl p-7 transition-all"
              >
        ...
        <section className="py-12 border-b border-white/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {CATEGORIES.map(cat => (
                <div key={cat.slug} className={`border rounded-xl p-4 cursor-pointer hover:opacity-90 transition-opacity ${cat.bg}`}>
        ...
        <section className="py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {benefits.map((b: { id: string; title: string; category: string; description: string; url: string }) => (
                <a
                  key={b.id}
                  href={b.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.06] hover:border-white/[0.1] rounded-xl p-5 transition-all"
                >
